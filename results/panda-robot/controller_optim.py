...
sys.path.insert(
    0, "/home/shiva/Documents/bo-prog/distributed-safe-bo/results/panda-robot/"
)
...
class SafeOpt_Optimizer(object):
    ...
    def __init__(
        self, upper_overshoot=0.08, upper_eigenvalue=-10, lengthscale=0.7, ARD=True
    ):
        self.upper_eigenvalue = upper_eigenvalue
        ...
        f -= self.mean_reward
        f /= self.std_reward
        print_str = f"params= {self.params}  | f={f:.4f}  | g1={g1:.2f}  | g2={g2:.2f}  | safe={True}"
        print(print_str)
        f = np.asarray([[f]])
        f = f.reshape(-1, 1)
        g1 = np.asarray([[g1]])
        ...
        KERNEL_f = GPy.kern.sde_Matern32(
            input_dim=x.shape[1], lengthscale=L, ARD=ARD, variance=1
        )
        KERNEL_g = GPy.kern.sde_Matern32(
            input_dim=x.shape[1], lengthscale=L, ARD=ARD, variance=1
        )
        gp0 = GPy.models.GPRegression(
            x[0, :].reshape(1, -1), f, noise_var=0.1 ** 2, kernel=KERNEL_f
        )
        gp1 = GPy.models.GPRegression(
            x[0, :].reshape(1, -1), g1, noise_var=0.1 ** 2, kernel=KERNEL_g
        )
        ...
        self.safety_list = [
            ...
            True,
            True,
        ]  # contains `i`th param is safe or not, 3 True for 3 initial safe points
    ...
    def optimize(self, i):
        ...
        self.safety_list.append(constraint_satisified)
        self.failures += constraint_satisified
        print_str = f"iter={i:3d}  | params= {param}  | f={f:.4f}  | g1={g1:.2f}  | g2={g2:.2f}  | safe={constraint_satisified}"
        print(print_str)
...
for r in range(runs):
    ...
    sim_data = opt.opt.data
    # print(sim_data)
    x = sim_data[0]
    y = sim_data[1]
    safe = np.array(opt.safety_list).reshape(-1, 1)
    a = pd.DataFrame(x)
    b = pd.DataFrame(y)
    c = pd.DataFrame(safe)
    ...
    points_df.to_csv(
        "/home/shiva/Documents/bo-prog/distributed-safe-bo/results-data/"
