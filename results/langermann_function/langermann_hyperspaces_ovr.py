...
class DeployHyperspace:
    ...
    def optimization(
        ...
        opt = safeopt.SafeOpt(
            gp, parameter_set, self.safe_threshold, lipschitz=None, threshold=0.2
        )
...
async def main():
    ...
    objective_function_name = "langermann_function"
    bounds = [(3, 5), (3, 5)]
    safe_threshold = -0.5
    # GP_regression noise
    noise_var = 0.05 ** 2
    ...
    objective_function = langermann_function
    # parameter_set = safeopt.linearly_spaced_combinations(bounds, 1000)
    no_subspaces = 2
    overlap = 0.2
    evaluation_constraint = 200
