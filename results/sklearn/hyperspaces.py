...
class DeployHyperspace:
    ...
    def optimization(
        ...
        try:
            ...
            while evaluation_constraint > 0:
                ...
                if (
                    y_meas >= self.safe_threshold
                    and new_hyperspace != current_hyperspace
                ):
                    break
    ...
    def deploy_hyperspace(self, hyperspace_no) -> None:
        ...
        # deploy_hyperspace.set_bounds(bounds, bounds_indices)
...
async def main():
    ...
    points_df.to_csv(
        "./results/" + objective_function_name + "_overlapped" + time_str + "_log.csv",
        index=False,
        header=False,
    )
    ...
    with open(
        "./results/" + objective_function_name + "_overlapped" + time_str + "_log.txt",
        "w",
    ) as res_file:
