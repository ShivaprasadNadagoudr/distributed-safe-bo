...
class DeployHyperspace:
    ...
    def __init__(
        ...
        if self.kernel_dict["name"] == "RBF":
            ...
            self.kernel = GPy.kern.Matern32(input_dim=self.kernel_dict["input_dim"])
        else:
            self.kernel = GPy.kern.RBF(
...
def sample_safe_fun(kernel_dict: Dict, bounds, noise_var, safe_threshold):
    if kernel_dict["name"] == "RBF":
        kernel = GPy.kern.RBF(
            input_dim=kernel_dict["input_dim"],
            variance=kernel_dict["variance"],
            lengthscale=kernel_dict["lengthscale"],
            ARD=kernel_dict["ARD"],
        )
    i = 0
    while True:
        fun = safeopt.sample_gp_function(kernel, bounds, noise_var, 50)
...
async def main():
    ...
    objective_function_name = "gp_sample"
    ...
    objective_function = sample_safe_fun(
        kernel_dict, bounds, noise_var_to_sample_function, safe_threshold
    )
    ...
    y_true = objective_function(parameter_set)
    scaler = MinMaxScaler()
    scaler.fit(y_true)
    y_scaled = pd.Series(np.squeeze(scaler.transform(y)))
    points_df = points_df.assign(y_scaled=y_scaled.values)
    y_unsafe = pd.Series(np.squeeze(y)).apply(lambda x: 1 if x < safe_threshold else 0)
    points_df = points_df.assign(y_unsafe=y_unsafe.values)

    points_df.to_csv(
        "./results/gp_sample_function/dbo_ovr/"
    ...
    # )
    # with open(
    #     "./results/gp_sample_function/dbo_ovr/" + objective_function_name + "_overlapped_" + time_str + "log.txt",
    #     "w",
    # ) as res_file:
