...
class DeployHyperspace:
    ...
    def optimization(
        ...
        opt = safeopt.SafeOpt(gp, parameter_set, self.safe_threshold, threshold=-20)
...
async def main():
    ...
    objective_function_name = "levy05_function"
    bounds = [(-2, 0), (-2, 0)]
    safe_threshold = -50.0
    # GP_regression noise
    noise_var = 0.5 ** 2
    kernel_dict = {
        ...
        "variance": 1.0,
        "lengthscale": [0.5, 0.25],
        "ARD": True,
    }
    objective_function = levy05_function
    # parameter_set = safeopt.linearly_spaced_combinations(bounds, 100)
    no_subspaces = 2
    overlap = 0.2
    evaluation_constraint = 150
