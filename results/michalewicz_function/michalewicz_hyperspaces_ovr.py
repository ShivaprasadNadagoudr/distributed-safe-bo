...
class DeployHyperspace:
    ...
    def optimization(
        ...
        opt = safeopt.SafeOptSwarm(
            gp, self.safe_threshold, bounds=self.bounds, threshold=0
        )
        # parameter_set = safeopt.linearly_spaced_combinations(self.bounds, 200)
        # opt = safeopt.SafeOpt(gp, parameter_set, self.safe_threshold)
...
async def main():
    ...
    objective_function_name = "michalewicz_function"
    bounds = [
        (0, np.pi),
        (0, np.pi),
        (0, np.pi),
    ...
    objective_function = michalewicz_function
    # parameter_set = safeopt.linearly_spaced_combinations(bounds, 1000)
    no_subspaces = 2
    overlap = 0.15
    evaluation_constraint = 350
    ...
    points_df.to_csv(
        "./results-data/" + objective_function_name + "_ovr_" + time_str + "log.csv",
        index=False,
    )
    ...
    with open(
        "./results-data/" + objective_function_name + "_ovr_" + time_str + "log.txt",
        "w",
    ) as res_file:
